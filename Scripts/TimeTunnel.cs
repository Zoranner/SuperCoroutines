...
// Description: Base on More Effective Coroutines v3.08.0
//============================================================
...
namespace Zoranner.SuperCoroutines
{
    public class TimeTunnel : MonoBehaviour
    {
        ...
        /// <summary>
        /// You can use "yield return TimeTunnel.WaitForOneFrame;" inside a coroutine function to go to the next frame. 
        /// </summary>
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        if (ex is MissingReferenceException)
                            Debug.LogError("This exception can probably be fixed by adding \"CancelWith(gameObject)\" when you run the coroutine.\n"
                                + "Example: TimeTunnel.RunCoroutine(_foo().CancelWith(gameObject), Segment.SlowUpdate);");
                    }
                }
            }
            ...
            if (_nextUpdateProcessSlot > 0)
            {
                ...
                for (coindex.i = 0; coindex.i < _lastUpdateProcessSlot; coindex.i++)
                {
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        if (ex is MissingReferenceException)
                            Debug.LogError("This exception can probably be fixed by adding \"CancelWith(gameObject)\" when you run the coroutine.\n"
                                + "Example: TimeTunnel.RunCoroutine(_foo().CancelWith(gameObject));");
                    }
                }
            }
            ...
            if (_nextFixedUpdateProcessSlot > 0)
            {
                ...
                for (coindex.i = 0; coindex.i < _lastFixedUpdateProcessSlot; coindex.i++)
                {
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        if (ex is MissingReferenceException)
                            Debug.LogError("This exception can probably be fixed by adding \"CancelWith(gameObject)\" when you run the coroutine.\n"
                                + "Example: TimeTunnel.RunCoroutine(_foo().CancelWith(gameObject), Segment.FixedUpdate);");
                    }
                }
            ...
            if (_nextLateUpdateProcessSlot > 0)
            {
                ...
                for (coindex.i = 0; coindex.i < _lastLateUpdateProcessSlot; coindex.i++)
                {
                    ...
                    catch (System.Exception ex)
                    {
                        ...
                        if (ex is MissingReferenceException)
                            Debug.LogError("This exception can probably be fixed by adding \"CancelWith(gameObject)\" when you run the coroutine.\n"
                                + "Example: TimeTunnel.RunCoroutine(_foo().CancelWith(gameObject), Segment.LateUpdate);");
                    }
                }
                currentCoroutine = default(CoroutineHandle);
            }
        ...
        /// <summary>
        /// Run a new coroutine on this TimeTunnel instance in the Update segment.
        /// </summary>
        /// <param name="coroutine">The new coroutine's handle.</param>
        /// <returns>The coroutine's handle, which can be used for Wait and Kill operations.</returns>
        ...
        /// <summary>
        /// Run a new coroutine on this TimeTunnel instance.
        /// </summary>
        /// <param name="coroutine">The new coroutine's handle.</param>
        /// <param name="segment">The segment that the coroutine should run in.</param>
        /// <returns>The coroutine's handle, which can be used for Wait and Kill operations.</returns>
        ...
        /// <summary>
        /// Kills the instance of the coroutine handle on this TimeTunnel instance if it exists.
        /// </summary>
        /// <param name="handle">The handle of the coroutine to kill.</param>
        /// <returns>The number of coroutines that were found and killed (0 or 1).</returns>
        ...
        /// <summary>
        /// Use "yield return TimeTunnel.WaitForSeconds(time);" to wait for the specified number of seconds.
        /// </summary>
        ...
        /// <summary>
        /// Use the command "yield return TimeTunnel.WaitUntilDone(otherCoroutine);" to pause the current 
        ...
        /// <summary>
        /// Use the command "yield return TimeTunnel.WaitUntilDone(otherCoroutine, false);" to pause the current 
        ...
        /// <summary>
        /// Use the command "yield return TimeTunnel.WaitUntilDone(wwwObject);" to pause the current 
        ...
        /// <summary>
        /// Use the command "yield return TimeTunnel.WaitUntilDone(operation);" to pause the current 
    ...
    public static class ExtensionMethods
    {
        ...
        {
            while (TimeTunnel.MainThread != System.Threading.Thread.CurrentThread || (gameObject && gameObject.activeInHierarchy && coroutine.MoveNext()))
